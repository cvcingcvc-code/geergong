...
这份文档解决一件事：**Windows 电脑开机、启动服务后，其他人通过一个公网 URL 就能用活动检索。**
...
# 窗口 1：本地服务（默认 127.0.0.1:8000，real 真实检索）
...
窗口 2 会打印一个 `https://<随机词>.trycloudflare.com`，把它发给任何人即可。
两个窗口都要保持开着；关掉任意一个，公网访问立刻失效。
...
|---|---|---|
| `--port` | `8000` | 监听端口 |
| `--host` | `127.0.0.1` | 监听地址。**不要改成 `0.0.0.0`**，见 F 节 |
...
```json
{"status": "ok", "providerMode": "demo", "demoData": true, "today": "2026-09-18"}
```
...
for p in /pipeline/data/review/review_queue.json /pipeline/data/approved/activities.json \
         ...
         /docs/DISTRICT_FILTER_REPORT.md /readme.md /ui_kits/ /pipeline/ ; do
...
```bash
# HTTP 边界（白名单、穿越、输入校验、限流、不泄漏）—— 会自己起临时端口，无需先开服务
...
**最快（推荐日常使用）**：回到 cloudflared 窗口按 `Ctrl+C`。

公网 URL 立刻失效，而本地服务不受影响，你仍可以继续用 `http://localhost:8000/` 自己调试。
...
3. `providers[]` 里哪个来源 `available:false`、`reason` 是什么（`timeout` / `http_error` / `not_configured` …）。
...
| 响应头 | `X-Content-Type-Options: nosniff`、`Referrer-Policy: no-referrer`、`X-Frame-Options: SAMEORIGIN`、CSP（`connect-src 'self'`） |
| 指纹 | 不播报 Python 版本，`Server: Gorgon` |
| 只读 | 唯一「写」接口是 `POST /api/search`，它不修改任何服务器状态；没有管理接口、没有 shell、没有命令执行 |

### 你必须自己保证的

1. **永远不要 `--debug` + 隧道同时开。** 那会让响应带上内部调试载荷。启动横幅会明确警告。
2. **不要用 `--host 0.0.0.0`。** 默认只听 `127.0.0.1`，公网一律经 cloudflared；绑 `0.0.0.0` 会让同网段的人绕过隧道直连。
3. **密钥不进仓库、不进截图、不进聊天记录。** 用环境变量；`SEARCH_API_KEY` 只在服务端使用，永远不会下发到浏览器。
4. **URL 就是凭证。** Quick Tunnel 没有鉴权，任何拿到 URL 的人都能用。泄漏了就 `Ctrl+C` 重启隧道换新 URL。
5. **限流是进程内的。** 重启即清零，也不跨进程共享；如果将来跑多实例或直接暴露在公网，需要换成外部限流。
6. **真实检索有出站成本。** 第三方来源的配额、速率限制与封禁风险由你承担；限流是为此加的第一道闸。
7. **前端依赖 CDN。** React/Babel/lucide 来自 `unpkg.com`，字体来自 `fonts.googleapis.com`。
   如果访问者的网络到不了这些域名，页面会白屏或字体降级。要完全自持，就把这几个文件下到本地并改 `index.html`。
8. **搜索结果是第三方的。** `trustScore` / 可信度标签只是提示，不构成保证；活动信息请以来源链接为准。
9. **跑服务的账户权限越小越好。** 建议用专用低权限账户，而不是管理员账户。
